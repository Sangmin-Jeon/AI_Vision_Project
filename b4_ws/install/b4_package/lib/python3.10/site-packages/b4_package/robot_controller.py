...
class RobotControllerNode(Node):
    ...
    # 차량을 따라가는 함수
    def follow_vehicle(self, x1, x2, frame_width):
        # 차량의 중앙 좌표 계산
        center_x = (x1 + x2) / 2
        ...
        angular_speed = -0.002 * error  # 중심에서의 오차를 바탕으로 회전 속도 설정
    ...
    def rotate_to_find_vehicle(self):
        twist = Twist()
        twist.linear.x = 0.0
        twist.angular.z = 0.5  # 반시계 방향으로 회전
        self.cmd_vel_publisher.publish(twist)
        ...
        for r in results:
            for box in r.boxes:
                ...
                if classNames[cls] == 'car' and confidence > 0.5:  # 특정 조건으로 필터링
                    ...
                    # 차량을 따라가기 위한 함수 호출
                    self.followh_veicle(x1, x2, frame.shape[1])
